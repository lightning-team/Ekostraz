...
export class LoaderComponent implements OnInit {
    ...
    private initDerivedTemplateObservables() {
        ...
        this.renderContent$ = loadingHidden$.pipe(
            map(([loading, renderHidden]) => !loading || renderHidden)
        );
        this.hidden$ = loadingHidden$.pipe(
            map(([loading, renderHidden]) => loading && renderHidden ? 'hidden' : '')
        );
    }
}
